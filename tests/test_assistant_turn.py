...
def test_non_act_modes_accept_text_only(mode: str) -> None:
    decision = AssistantTurnDecision(
        mode=mode,
        assistant_text="我们先想一想。",
        help_seeking=mode in {"hint", "diagnose"},
    )
...
def test_decide_circuit_turn_tool_uses_strict_schema() -> None:
    ...
    assert "help_seeking" in function["parameters"]["required"]


def test_help_seeking_is_independent_from_action_mode() -> None:
    decision = AssistantTurnDecision(
        mode="hint",
        assistant_text="先看看两块积木有什么不同。",
        help_seeking=True,
    )

    assert decision.help_seeking is True
    assert decision.candidate_id is None
