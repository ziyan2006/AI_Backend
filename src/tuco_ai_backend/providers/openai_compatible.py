...
LEVEL_CHILD_GUIDANCE = {
    101: LevelChildGuidance(
        "这关要让一个开关直接控制输出积木，开关亮时输出也亮，灭时也灭。",
        "让开关的亮灭直接传到输出积木。",
        "直连",
        opening_instruction=(
            "当孩子问本关做什么，先说让开关的亮灭直接传到输出积木；"
            "不得对孩子复述 A、Y 等信号标签，也不要提前讲具体接线。"
        ),
    ),
    ...
    602: LevelChildGuidance(
        ...
        "两个开关的不同组合，每次只让四个舱室中的一个亮起。",
        "二转四译码器",
        action_instruction=(
            "第一行必须说“两个只会是0或1的开关，每次只让四个舱室中的一个亮起”；"
            "第二行只邀请摆放一块非门积木。"
        ),
    ),
}
...
def build_level_child_guidance_instruction_for_level(
    ...
    *,
    ...
    if has_actionable_circuit_progress:
        ...
        if level_id == 502:
            semantic_priority_instruction += (
                "本关或门接收前面与门产生的结果；只要这些结果中有一路亮，或门结果就亮；"
                "不得说成直接判断原始开关，也不得说任意一个原始开关亮就会产生进位。"
            )
        if level_id == 601:
            semantic_priority_instruction += (
                "本关中与门的白话作用是：送进与门的两个信号同时亮时，这一路才通过；"
                "不得只说“满足控制条件”，也不得把与门描述为汇总结果；"
    ...
    if has_actionable_circuit_progress:
        action_response_instruction = (
            "如果意图是开始行动，直接围绕实时安全候选给当前唯一的一小步，"
            "不得先盘点已完成部分，也不得以已有积木数量开场；"
            "必须以电路进度判断为准，不能套用固定摆放顺序。"
        )
    else:
        action_response_instruction = (
            "如果意图是开始行动，第一行自然承接当前进度，第二行只给当前唯一的下一步动作；"
            "此时不应注入与当前状态冲突的固定摆放步骤。"
        )
    return (
