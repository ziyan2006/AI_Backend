...
const LEVEL_DATA = {
  ...
  501: { level_id: 501, title: "个位引擎", short_goal: "全加器个位求和：只有当输入 1 的个数为奇数个（1或3个）时个位输出 1。", input_names: "加数 A, B, C", output_names: "个位 Sum", input_count: 3, output_count: 1, truth_table: [{ in: [0, 0, 0], out: [0] }, { in: [0, 0, 1], out: [1] }, { in: [0, 1, 0], out: [1] }, { in: [0, 1, 1], out: [0] }, { in: [1, 0, 0], out: [1] }, { in: [1, 0, 1], out: [0] }, { in: [1, 1, 0], out: [0] }, { in: [1, 1, 1], out: [1] }] },
  502: { level_id: 502, title: "进位引擎", short_goal: "全加器进位计算：只要任意 2 个或 2 个以上输入同为 1，就产生进位。", input_names: "加数 A, B, C", output_names: "进位", input_count: 3, output_count: 1, truth_table: [{ in: [0, 0, 0], out: [0] }, { in: [0, 0, 1], out: [0] }, { in: [0, 1, 0], out: [0] }, { in: [0, 1, 1], out: [1] }, { in: [1, 0, 0], out: [0] }, { in: [1, 0, 1], out: [1] }, { in: [1, 1, 0], out: [1] }, { in: [1, 1, 1], out: [1] }] },
  503: { level_id: 503, title: "全加主控", short_goal: "组合个位计算与进位计算，实现一个三输入的全加器。", input_names: "加数 A, B, 进位 C", output_names: "Sum, Carry", input_count: 3, output_count: 2, truth_table: [{ in: [0, 0, 0], out: [0, 0] }, { in: [0, 0, 1], out: [1, 0] }, { in: [0, 1, 0], out: [1, 0] }, { in: [0, 1, 1], out: [0, 1] }, { in: [1, 0, 0], out: [1, 0] }, { in: [1, 0, 1], out: [0, 1] }, { in: [1, 1, 0], out: [0, 1] }, { in: [1, 1, 1], out: [1, 1] }] },
