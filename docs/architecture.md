...
- 浏览器测试台、PCM 模拟提交和 Python 设备模拟器。

当前 ASR 实现会先在后端缓冲一轮录音，在 `input_audio.commit` 后再向火山分块发送；它与现有长按说话固件兼容，但不是录音期间的实时 ASR 流。后续若要降低松手后的识别延迟，可在不改变设备播放逻辑的前提下，将供应商 ASR 会话提前到 `input_audio.start` 创建。
